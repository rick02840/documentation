...
#===============================================================#
rsync --size-only --delete-after -rlzi --ipv4 --progress --info=BACKUP,DEL -e 'ssh -t -p 2222 -oStrictHostKeyChecking=no' output_prod/docs/ --temp-dir=../../tmp/ live.$PROD_UUID@appserver.live.$PROD_UUID.drush.in:files/docs/
if [ "$?" -eq "0" ]
then
